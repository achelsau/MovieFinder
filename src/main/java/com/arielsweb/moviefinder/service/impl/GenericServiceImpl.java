...
public abstract class GenericServiceImpl<T> implements GenericService<T> {
	...
	public GenericServiceImpl() {
		...
		this.entityBeanType = ((Class<?>) ((ParameterizedType) getClass().getGenericSuperclass())
	...
	public T find(Long id) {
		try {
			SQLQuery query = createSQLQuery("SELECT * from " + tableName + " WHERE id = " + id + ";").addEntity(
					entityBeanType);
	...
	public void delete(Long id) {
		if (id > 0) {
			SQLQuery query = createSQLQuery("DELETE from " + tableName + " WHERE id =" + id + ";");
			query.executeUpdate();
		} else {
