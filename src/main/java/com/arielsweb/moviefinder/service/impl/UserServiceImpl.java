...
public class UserServiceImpl extends GenericServiceImpl<User> implements UserService {
	...
	public void setOnline(Long id) {
		if (id > 0) {
			SQLQuery query = createSQLQuery("UPDATE " + tableName + " SET " + MovieFinderConstants.USER_IS_ONLINE
					+ "=1" + " WHERE id=" + id + ";");
			query.executeUpdate();
		} else {
			log.debug(this.getClass() + ":" + MovieFinderConstants.INCORRECT_ID_UPDATE_ERROR);
			throw new DataAccessException(this.getClass() + ":" + MovieFinderConstants.INCORRECT_ID_UPDATE_ERROR);
		}
	}
	...
	public User getUserByUsername(String username) {
		if (username != null && !username.isEmpty()) {
			SQLQuery query = createSQLQuery("SELECT * from " + tableName + " WHERE username='" + username + "';")
			...
			log.debug(this.getClass() + ":" + MovieFinderConstants.INCORRECT_GET_BY_USERNAME);
			throw new DataAccessException(this.getClass() + ":" + MovieFinderConstants.INCORRECT_GET_BY_USERNAME);
		}
	}
