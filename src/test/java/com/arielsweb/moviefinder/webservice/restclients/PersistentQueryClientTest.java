...
public class PersistentQueryClientTest {
	...
	public void quickQuery() {
		...
		// 1. Run a quick query
		ResultInfoResponse resultInfoResponse = QueryEngineUtils
				.quickQueryForMovie(queryString);
	...
	public void persistQueryAndRemoveIt() {
		...
		// 1. Persist a query
		ResultInfoResponse resultInfoResponse = QueryEngineUtils
				.sendQueryToServer(queryString, interval, null);
		// this means it was persisted
		assertNotNull(resultInfoResponse.getQueryId());

		// 2. Remove the persistent query
		int responseCode = QueryEngineUtils
				.removePersistentQuery(resultInfoResponse.getQueryId());
	...
	public void persistQueryAndUpdateIt() {
		...
		resultInfoResponse = QueryEngineUtils.sendQueryToServer(queryString,
				interval, idAfterStorage);
